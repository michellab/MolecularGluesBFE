...
dof = str(sys.argv[2]) # Select CRBN, CK1a, CRBN_only, CK1awithCRBN
...
if species == 'CK1a':
    ...
    prmtop_filename = 'CRBN.prmtop'
    inpcrd_filename = 'CRBN.inpcrd'    

elif species in ['CRBN_only', 'CK1a_only', 'CRBNwithCK1a', 'CK1awithCRBN']:
    prmtop_filename = 'complex_eq.prmtop'
    inpcrd_filename = 'complex_eq.inpcrd'

else:
    raise FileNotFoundError(f"Select one of the following options for species of interest: CRBN, CK1a, CRBN_only, CK1a_only, CRBNwithCK1a, CK1awithCRBN")    
...
elif species == 'CRBN' or species == 'CRBN_only':
    receptor_atoms = [
        atom.index for atom in simulation.topology.atoms()
...
else: # CK1awithCRBN or CRBNwithCK1a
    receptor_atoms = [
        atom.index for atom in simulation.topology.atoms()
        if atom.residue.index in range(0, 312) and atom.name in restraint_selection
    ]
...
if species in ['CRBN', 'CRBN_only', 'CRBNwithCK1a']:
...
if species in ['CK1a', 'CK1awithCRBN', 'CK1a_only']:
    for atom in simulation.topology.atoms():
