...
if species in ['BD1_only_bulk', 'BD2_IBG1withBD1_bulk', 'BD2_IBG1_only_bulk', 'BD1withBD2_IBG1_bulk']:
    prmtop_filename= 'BRD4G386E_IBG1.prmtop'
    inpcrd_filename = 'BRD4G386E_IBG1.inpcrd'
    ...
    raise FileNotFoundError(f"Select one of the following options for species of interest: DCAF16, BRD4G318E, DCAF16_only, BRD4G318E_only, DCAF16withBRD4G318E, BRD4G318EwithDCAF16")    
...
if 'BD1' in species:
    for atom in simulation.topology.atoms():
        if atom.index==BD1_atoms[0] and atom.residue.name!='THR':
            raise ValueError(f'Incorrect residue selection for BRD4G318E - residue THR174 is missing')
        if atom.index==BD1_atoms[-1] and atom.residue.name!='THR':
            raise ValueError(f'Incorrect residue selection for BRD4G318E - residue THR280 is missing')
        
if 'BD2_IBG1' in species:
    for atom in simulation.topology.atoms():
        if atom.index==BD2_IBG1_atoms[0] and atom.residue.name!='LYS':
            raise ValueError(f'Incorrect residue selection for BRD4G318E - residue LYS281 is missing')
