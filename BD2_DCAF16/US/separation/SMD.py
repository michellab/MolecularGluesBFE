...
ligand_atoms = [
    atom.index for atom in simulation.topology.atoms()
    if atom.residue.index in range(173, 281) and atom.name in ('CA', 'C', 'N')
]
...
cv.addGroup(rec_group)
cv.addGroup(lig_group)

# Specify bond groups
bondGroups = [0, 1]
cv.addBond(bondGroups)

r_0 = 1.1 * unit.nanometers #Set initial separation of 11.0 Angstrom
...
theta_A_pot.addGroup(rec_group)
theta_A_pot.addGroup(lig_group)
...
phi_A_pot.addGroup(rec_group)
phi_A_pot.addGroup(lig_group)
...
phi_B_pot.addGroup(rec_group)
phi_B_pot.addGroup(lig_group)
phi_B_pot.addGroup([idx_B])
...
# Add the particle groups
phi_C_pot.addGroup(rec_group)
phi_C_pot.addGroup(lig_group)
...
n_red = 1000 # Total number of steps to reduce r_0 over
