...
dof = str(sys.argv[2]) # Select CRBN_len, CK1aI35G, CRBN_only, CK1aI35GwithCRBN
...
if species == 'CK1aI35G': # Unbound
    prmtop_filename= 'CK1aI35G.prmtop'
    inpcrd_filename = 'CK1aI35G.inpcrd'

elif species == 'CRBN_len': # Unbound
    prmtop_filename = 'CRBN_len.prmtop'
    inpcrd_filename = 'CRBN_len.inpcrd'    

elif species in ['CRBN_len_only', 'CK1aI35G_only', 'CRBN_lenwithCK1aI35G', 'CK1aI35GwithCRBN_len']: 
    prmtop_filename = 'complex_eq.prmtop' # Bound
    inpcrd_filename = 'complex_eq.inpcrd'

else:
    raise FileNotFoundError(f"Select one of the following species: CRBN_len, CK1aI35G, CRBN_len_only, CK1aI35G_only, CRBN_lenwithCK1aI35G, CK1aI35GwithCRBN_len")    
...
elif species == 'CK1aI35G_only':
    ligand_atoms = [
        atom.index for atom in simulation.topology.atoms()
        if atom.residue.index in range(313, 606) and atom.name in restraint_selection
    ]

else: # CK1aI35GwithCRBN_len or CRBN_lenwithCK1aI35G
    receptor_atoms = [
        atom.index for atom in simulation.topology.atoms()
        if atom.residue.index in range(0, 312) and atom.name in restraint_selection
    ]
...
if species in ['CRBN_len', 'CRBN_len_only', 'CRBN_lenwithCK1aI35G']:
...
if species in ['CK1aI35G', 'CK1aI35GwithCRBN_len', 'CK1aI35G_only']:
    for atom in simulation.topology.atoms():
        if atom.index==ligand_atoms[0] and atom.residue.name!='PHE':
            raise ValueError(f'Incorrect residue selection for CK1aI35G - residue F314 is missing')
        if atom.index==ligand_atoms[-1] and atom.residue.name!='GLN':
            raise ValueError(f'Incorrect residue selection for CK1aI35G - residue Q606 is missing')

"""Applying RMSD forces"""

if species in ['CK1aI35G', 'CK1aI35G_only']:
...
elif species == 'CRBN_lenwithCK1aI35G':
    receptor_rmsd_force = mm.CustomCVForce('0.5*k_rec*(rmsd-rmsd_0)^2')
    receptor_rmsd_force.addGlobalParameter('rmsd_0', float(RMSD_0) * unit.angstrom)
    receptor_rmsd_force.addGlobalParameter('k_rec', k_RMSD * unit.kilocalories_per_mole / unit.angstrom**2)
    receptor_rmsd_force.addCollectiveVariable('rmsd', mm.RMSDForce(reference_positions, receptor_atoms))
