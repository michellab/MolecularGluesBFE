...
ligand_atoms = [
    atom.index for atom in simulation.topology.atoms()
    if atom.residue.index in range(173, 391) and atom.name in ('CA', 'C', 'N')
]

# Add IBG1 heavy atom indices to restrained ligand atoms
ligand_atoms = ligand_atoms + IBG1_indices
...
for atom in simulation.topology.atoms():
    ...
    if atom.index==ligand_atoms[-60] and atom.residue.name!='ASP':
        raise ValueError(f'Incorrect residue selection for BRD4 - residue D391 is missing')
    if atom.index==ligand_atoms[-1] and atom.residue.name!='MOL':
...
dr = (0.2/n_red) * unit.nanometers # Bring separation down
