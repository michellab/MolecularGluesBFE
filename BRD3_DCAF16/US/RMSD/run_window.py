...
if species in ['BD1_only_bulk', 'BD2withBD1_bulk', 'BD2_only_bulk', 'BD1withBD2_bulk']:
    prmtop_filename= 'BRD3.prmtop'
    inpcrd_filename = 'BRD3.inpcrd'

elif species == 'DCAF16': 
    prmtop_filename = 'DCAF16.prmtop'
    inpcrd_filename = 'DCAF16.inpcrd'    

elif species in ['DCAF16_only', 'BD1withDCAF16', 'BD2withDCAF16andBD1', 'BD2withDCAF16', 'BD1withDCAF16andBD2', 'BD1_only', 'BD2withBD1', 'DCAF16withBD1andBD2', 'DCAF16withBD1', 'BD2withDCAF16andBD1', 'BD2_only', 'BD1withBD2', 'DCAF16withBD1andBD2', 'DCAF16withBD2', 'BD1withDCAF16andBD2']:
    prmtop_filename = 'complex_eq.prmtop'
    inpcrd_filename = 'complex_eq.inpcrd'

else:
    raise FileNotFoundError(f"Select one of the following options for species of interest: DCAF16, BRD3, DCAF16_only, BRD3_only, DCAF16withBRD3, BRD3withDCAF16")    
...
# Bulk state BD1 and BD2 selections
if species in ['BD1_only_bulk', 'BD2_only_bulk', 'BD1withBD2_bulk', 'BD2withBD1_bulk']:
    BD1_atoms = [
    ...
    BD2_atoms = [
        atom.index for atom in simulation.topology.atoms()
        if atom.residue.index in range(107, 217) and atom.name in restraint_selection
    ]

# Bulk state DCAF16 + complex selections
if species in ['DCAF16', 'DCAF16_only', 'BD1withDCAF16', 'BD2withDCAF16andBD1', 'BD2withDCAF16', 'BD1withDCAF16andBD2', 
    'BD1_only', 'BD2withBD1', 'DCAF16withBD1andBD2', 'DCAF16withBD1', 'BD2withDCAF16andBD1', 'BD2_only', 
    'BD1withBD2', 'DCAF16withBD1andBD2', 'DCAF16withBD2', 'BD1withDCAF16andBD2']:
    receptor_atoms = [
        atom.index for atom in simulation.topology.atoms()
    ...
    DDB1_binding_atoms = [
        atom.index for atom in simulation.topology.atoms()
        if atom.residue.index in DCAF16_DDB1_binding_residx and atom.name in ['C', 'N', 'CA']
    ]
    BD1_atoms = [
        atom.index for atom in simulation.topology.atoms()
...
if 'BD1' in species:
    for atom in simulation.topology.atoms():
        if atom.index==BD1_atoms[0] and atom.residue.name!='THR':
            raise ValueError(f'Incorrect residue selection for BRD3 - residue THR174 is missing')
        if atom.index==BD1_atoms[-1] and atom.residue.name!='GLN':
            raise ValueError(f'Incorrect residue selection for BRD3 - residue GLN280 is missing')
        
if 'BD2' in species:
    for atom in simulation.topology.atoms():
        if atom.index==BD2_atoms[0] and atom.residue.name!='LYS':
            raise ValueError(f'Incorrect residue selection for BRD3 - residue LYS281 is missing')
        if atom.index==BD2_atoms[-1] and atom.residue.name!='PRO':
            raise ValueError(f'Incorrect residue selection for BRD3 - residue PRO390 is missing')
...
# BD1 CV
if species in ['BD1_only_bulk', 'BD1withBD2_bulk', 'BD1withDCAF16', 'BD1withDCAF16andBD2', 'BD1_only', 
        'BD1withBD2', 'BD1withDCAF16andBD2']:
...
# BD2 CV
elif species in ['BD2withBD1_bulk', 'BD2_only_bulk', 'BD2withDCAF16andBD1', 'BD2withDCAF16', 'BD2withBD1',
        'BD2withDCAF16andBD1', 'BD2_only']:
    ligand_rmsd_force = mm.CustomCVForce('0.5*k_lig*(rmsd-rmsd_0)^2')
    ligand_rmsd_force.addGlobalParameter('rmsd_0', float(RMSD_0) * unit.angstrom)
    ligand_rmsd_force.addGlobalParameter('k_lig', k_RMSD * unit.kilocalories_per_mole / unit.angstrom**2)
    ligand_rmsd_force.addCollectiveVariable('rmsd', mm.RMSDForce(reference_positions, BD2_atoms))
    system.addForce(ligand_rmsd_force)

# DCAF16 CV
elif species in ['DCAF16', 'DCAF16_only', 'DCAF16withBD1andBD2', 'DCAF16withBD1', 'DCAF16withBD1andBD2', 
        'DCAF16withBD2']:
...
# BD1 restrained
if species in ['BD2withBD1_bulk', 'BD2withDCAF16andBD1', 'BD2withBD1', 'DCAF16withBD1andBD2', 'DCAF16withBD1', 
        'BD2withDCAF16andBD1', 'DCAF16withBD1andBD2']:
...
# BD2 restrained
if species in ['BD1withBD2_bulk', 'BD1withDCAF16andBD2', 'DCAF16withBD1andBD2', 'BD1withBD2', 'DCAF16withBD1andBD2', 
        'DCAF16withBD2', 'BD1withDCAF16andBD2' ]:
    BD2_rmsd_force = mm.CustomCVForce('0.5*k_BD2*rmsd^2')
    BD2_rmsd_force.addGlobalParameter('k_BD2', k_RMSD * unit.kilocalories_per_mole / unit.angstrom**2)
    BD2_rmsd_force.addCollectiveVariable('rmsd', mm.RMSDForce(reference_positions, BD2_atoms))
    system.addForce(BD2_rmsd_force)

# DCAF16 restrained
if species in ['BD1withDCAF16', 'BD2withDCAF16andBD1', 'BD2withDCAF16', 'BD1withDCAF16andBD2', 'BD2withDCAF16andBD1',
        'BD1withDCAF16andBD2']:
...
for i in range(sampling_steps//record_steps):
    ...
    if species in ['DCAF16', 'DCAF16_only', 'DCAF16withBD1andBD2', 'DCAF16withBD1', 
        'DCAF16withBD1andBD2', 'DCAF16withBD2']:
