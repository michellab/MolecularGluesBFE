...
for atom in simulation.topology.atoms():
    if atom.index==receptor_atoms[0] and atom.residue.name!='ASN':
        raise ValueError(f'Incorrect residue selection for DCAF16 - residue ASN1 is missing')
    if atom.index==receptor_atoms[-1] and atom.residue.name!='LEU':
        raise ValueError(f'Incorrect residue selection for DCAF16 - residue LEU172 is missing')
...
def obtain_RMSDs(simulation):
    ...
    RMSD_BD2 = 10*BD2_rmsd_force.getCollectiveVariableValues(simulation.context)[0]
    RMSD_DDB1 = 10*DDB1_rmsd_force.getCollectiveVariableValues(simulation.context)[0]

    return [RMSD_rec, RMSD_BD1, RMSD_BD2, RMSD_DDB1]
