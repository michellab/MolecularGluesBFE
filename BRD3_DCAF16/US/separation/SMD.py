...
receptor_atoms = [
    ...
    if atom.residue.index in receptor_residues and atom.name in ('CA', 'C', 'N')
]
ligand_atoms = [
    atom.index for atom in simulation.topology.atoms()
    if atom.residue.index in ligand_residues and atom.name in ('CA', 'C', 'N')
]
...
for atom in simulation.topology.atoms():
    ...
    if atom.index==ligand_atoms[-1] and atom.residue.name!='PRO':
        raise ValueError(f'Incorrect residue selection for BRD3 - residue P390 is missing')
