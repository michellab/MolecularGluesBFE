...
if species in ['BD1_only_bulk', 'BD2_IBG1withBD1_bulk', 'BD2_IBG1_only_bulk', 'BD1withBD2_IBG1_bulk']:
    prmtop_filename= 'BRD3E344G_IBG1.prmtop'
    inpcrd_filename = 'BRD3E344G_IBG1.inpcrd'
    ...
    raise FileNotFoundError("RMSD species not recognised!")    
...
if 'BD1' in species:
    for atom in simulation.topology.atoms():
        if atom.index==BD1_atoms[0] and atom.residue.name!='THR':
            raise ValueError(f'Incorrect residue selection for BRD3E318G - residue THR174 is missing')
        if atom.index==BD1_atoms[-1] and atom.residue.name!='GLN':
            raise ValueError(f'Incorrect residue selection for BRD3E318G - residue THR280 is missing')
        
if 'BD2_IBG1' in species:
    for atom in simulation.topology.atoms():
        if atom.index==BD2_IBG1_atoms[0] and atom.residue.name!='LYS':
            raise ValueError(f'Incorrect residue selection for BRD3E318G - residue LYS281 is missing')
